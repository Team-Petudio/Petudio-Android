package com.composition.damoa.data.network.retrofit.callAdapter
...
class PetudioCall<T : Any>(
    ...
    override fun enqueue(callback: Callback<ApiResponse<T>>) {
        call.enqueue(object : Callback<T> {
            override fun onResponse(call: Call<T>, result: Response<T>) {
                ...
                callback.onResponse(this@PetudioCall, Response.success(apiResult))
            }
    ...
    override fun clone(): Call<ApiResponse<T>> = PetudioCall<T>(call.clone(), responseType)
    ...
    companion object {
        private const val NOT_SUPPORT_EXECUTE = "[ERROR] PetudioCall은 execute를 지원하지 않습니다."
